...
public class SpringChain implements SpringListener {
    ...
    public SpringChain addSpring(int position, SpringListener listener) {
        if (mSprings.size() == position) {
            mSprings.add(position,
                    mSpringSystem.createSpring()
                            .addListener(this)
                            .setSpringConfig(this.mAttachmentSpringConfig));
        } else if (mSprings.size() < position) {
        }
        if (mListeners.size() > position) {
            mListeners.set(position, listener);
        } else if (mListeners.size() == position) {
            mListeners.add(position, listener);
        }
        return this;
    }
    ...
    public void onSpringChainUpdate(Spring spring, Spring springControl) {
        ...
        if (idx >= 0) {
            ...
            Spring controlSpring = mSprings.get(mControlSpringIndex);
            if (listener != null) {
                listener.onSpringChainUpdate(spring, controlSpring);
            }
        }
    }
